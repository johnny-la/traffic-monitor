...
//			System.out.println("CurrentTime = " + (currentTime-startTime));
		...
		System.out.println("Current RPS = " + metricManager.getMonitorRps() + ", timeDiff = " + (currentTime-startTime));
		metricManager.monitorThroughput((long)currentTime);
		System.out.println("Current RPS = " + metricManager.getMonitorRps() + ", timeDiff = " + (currentTime-startTime));
		alertEquals(new Alert[] {
		        new Alert(thresholdRequestCount, false)}, 
		...
		alertEquals(new Alert[]{
				new Alert(thresholdRequestCount, false),
				new Alert(thresholdRequestCount-1, true)},
				metricManager.getAlerts());
		...
		alertEquals(new Alert[]{
				new Alert(thresholdRequestCount, false),
				new Alert(thresholdRequestCount-1, true),
				new Alert(thresholdRequestCount*5, false)},
				metricManager.getAlerts());
