...
- Create an `AlertSchema` class that stores rules and templates for triggering alerts. For instance, an `AlertSchema` could store a critical RPS threshold which, when surpassed, would trigger an alert. To monitor the metrics, the `MetricMonitor` would hold a list of `AlertSchemas`, which would then be compared with metrics to trigger appropriate alerts. As a result, different rules could be used in the same `MetricMonitor`, thus making monitors more unit-testable and extensible
